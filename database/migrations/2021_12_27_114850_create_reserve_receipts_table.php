...
class CreateReserveReceiptsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('reserve_receipts', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedInteger('agency_id')->comment('会社ID');
            $table->unsignedBigInteger('reserve_id')->comment("予約ID");
            $table->unsignedBigInteger('reserve_invoice_id')->comment('請求ID');
    ...
    {
        Schema::dropIfExists('reserve_receipts');
    }
}

 ...
 * 仕入参加者料金（オプション科目用）
 */
class CreateReserveParticipantOptionPricesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('reserve_participant_option_prices', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('reserve_itinerary_id')->comment('行程ID');
            $table->unsignedBigInteger('reserve_purchasing_subject_option_id')->comment('オプション科目ID');
            ...
            $table->foreign('reserve_purchasing_subject_option_id')
                ...
                ->onDelete('cascade')->name('reserve_purchasing_subject_option_foreign');
    ...
    {
        Schema::dropIfExists('reserve_participant_option_prices');
    }
}

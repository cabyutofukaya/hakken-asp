...
class CreateSubjectHotelsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('subject_hotels', function (Blueprint $table) {
            ...
            $table->string('hotel_name')->nullable()->comment('ホテル名');
            $table->string('address')->nullable()->comment('住所');
            $table->string('tel')->nullable()->comment('電話番号');
            $table->string('fax')->nullable()->comment('FAX番号');
            $table->string('url')->nullable()->comment('URL');
            $table->unsignedBigInteger('city_id')->nullable()->comment('都市・空港');
        ...
        DB::statement('ALTER TABLE subject_hotels MODIFY code varchar(256) BINARY'); // 商品コードは、大文字小文字を区別させる。
    }
    ...
    {
        Schema::dropIfExists('subject_hotels');
    }
}

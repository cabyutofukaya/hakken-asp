...
/**
 * 会社情報編集フォームに使う選択項目などを提供するViewComposer
 */
class EditFormComposer
{
    ...
    public function compose(View $view)
    {
        ...
        $agency = Arr::get($data, 'agency');

        /////////////

        $defaultValue = array_merge($agency->toArray(), session()->getOldInput());
        ...
        $consts = [
            ...
            'pdfBaseUrl' => \Storage::disk('s3')->url(config('consts.const.UPLOAD_PDF_DIR')),
        ];

        $view->with(compact('formSelects', 'statuses', 'businessScopes', 'registrationTypes', 'travelAgencyAssociations', 'prefectureCodes', 'contractPlans','defaultValue','consts'));
    }
}

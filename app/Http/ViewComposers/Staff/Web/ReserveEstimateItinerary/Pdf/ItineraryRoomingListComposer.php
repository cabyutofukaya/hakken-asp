        ...
        foreach ($reserveItinerary->reserve_travel_dates as $reserveTravelDate) {
            ...
            foreach ($reserveTravelDate->reserve_schedules as $reserveSchedule) {
                foreach ($reserveSchedule->reserve_purchasing_subject_hotels as $reservePurchasingSubjectHotel) {
                    if (!isset($roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name])) {
                        $roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name] = [];
                    }
                    // 部屋タイプ情報
                    $roomType = data_get($reservePurchasingSubjectHotel, 'room_types.0.val', '');
                    if (!isset($roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name][$roomType])) {
                        $roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name][$roomType] = [];
                    }

                    foreach ($reservePurchasingSubjectHotel->reserve_participant_prices as $reserveParticipantPrice) {

                        // 部屋タイプと部屋番号の区別で分ける
                        if (!isset($roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name][$roomType][$reserveParticipantPrice->room_number])) {
                            $roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name][$roomType][$reserveParticipantPrice->room_number] = [];
                        }
                        if ($reserveParticipantPrice->valid) {
                            $roomingList[$reserveTravelDate->travel_date][$reservePurchasingSubjectHotel->name][$roomType][$reserveParticipantPrice->room_number][] = $reserveParticipantPrice->participant;
                        }
                    }
                }
            }

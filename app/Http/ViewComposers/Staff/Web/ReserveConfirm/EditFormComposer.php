...
class EditFormComposer
{
    ...
    public function compose(View $view)
    {
        ...
        if ($reserveConfirm) { // 保存データあり。$reserveConfirmから各種データをセット
            ...
            // 参加者情報
            $participantInfo = $this->webReserveService->getParticipants($reserveConfirm->reserve_itinerary->reserve->id, true); // 参加者一覧（取消者を含む。削除済は取得しない）
            ...
            // 参加者情報
            $participantInfo = $this->webReserveService->getParticipants($reserveItinerary->reserve_id, true); // 参加者一覧（取消者を含む。削除済は取得しない）
        ...
        $view->with(compact('reception', 'defaultValue', 'formSelects', 'consts', 'documentCommonSetting', 'hotelContacts', 'hotelInfo', 'optionPrices', 'airticketPrices', 'hotelPrices', 'documentSetting', 'jsVars', 'isDeparted', 'isCanceled'));
    }
}

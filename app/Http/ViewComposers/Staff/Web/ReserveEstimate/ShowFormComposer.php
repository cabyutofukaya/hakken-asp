...
class ShowFormComposer
{
    ...
    public function compose(View $view)
    {
        ...
        // メッセージに使うRoomキー
        $roomKey = $reserve->web_reserve_ext->room_key;
        ...
        $defaultValue = [
            ...
            config('consts.reserves.TAB_BASIC_INFO') => [
                ...
                'updatedAt' => $reserve->updated_at->format('Y-m-d H:i:s'), // 予約情報更新日時
            ],
        ...
        $participantCancelChargeUrlPattern = $applicationStep === config('consts.reserves.APPLICATION_STEP_RESERVE') ? route("staff.web.estimates.reserve.participant_cancel_charge.edit", [$agencyAccount, $reserve->control_number, "#id#"]) . $departedQuery : ''; // 予約状態の場合は参加者チャンセルチャージURL(ID部分はjavascriptで変更)
        ...
        $afterCancelUrl = $applicationStep === config('consts.reserves.APPLICATION_STEP_RESERVE') ? route('staff.web.estimates.reserve.show', [$agencyAccount, $reserve->control_number]) : ''; // キャンセル後の転送先(キャンセルチャージナシ時)
        ...
        $consts = [
            ...
            [
                ...
                'determineUrl' => route('staff.api.web.estimate.determine', [$agencyAccount, $reserve->estimate_number]), // 見積決定URL

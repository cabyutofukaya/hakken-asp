 ...
 * 領収書データ(一括用)
 */
class ReserveBundleReceipt extends Model implements DocumentPdfInterface
{
    ...
    {
        $values = Lang::get('values.reserve_bundle_receipts.status');
        foreach (config("consts.reserve_bundle_receipts.STATUS_LIST") as $key => $val) {
            if ($val == $this->status) {

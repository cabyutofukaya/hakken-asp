<div id="inputArea" class="pt10">
  ...
  <h2 class="mb30 documentSubTit pt00"><span class="material-icons">subject</span>出力項目設定</h2>
    <ul class="baseList">
      <li class="wd40">
        <span class="inputLabel req">表題</span>
    ...
    <ul class="documentCheckList">
      <li>
        <h3>{{ __("values.document_request_alls.display_block") }}</h3>
        <ul>
          <input type="hidden" name="setting[{{ config('consts.document_request_alls.DISPLAY_BLOCK') }}]" />{{-- チェックなしでも値が更新されるようにhiddenで初期化 --}}
          @foreach(array_filter(config('consts.document_request_alls.DISPLAY_BLOCK_LIST'), function ($key) {
            return $key !== '検印欄';
          }, ARRAY_FILTER_USE_KEY) as $parent => $childs) {{-- 「検印欄」は除外 --}}
            @include('staff.document_category.common._setting_row', [
              'checkValues' => Arr::get($defaultValue, 'setting.' . config('consts.document_request_alls.DISPLAY_BLOCK'), null),
              'parent' => $parent,
              'childs' => $childs,
              'name' => config('consts.document_request_alls.DISPLAY_BLOCK'),
              'idPrefix' => 'db',
            ])
          @endforeach
        </ul>
      ...
      <li>
        <h3>{{ __("values.document_request_alls.reservation_info") }}</h3>
        <ul>
          <input type="hidden" name="setting[{{ config('consts.document_request_alls.RESERVATION_INFO') }}]" />{{-- チェックなしでも値が更新されるようにhiddenで初期化 --}}
          @foreach(config('consts.document_request_alls.RESERVATION_INFO_LIST') as $parent => $childs)
            @include('staff.document_category.common._setting_row', [
              'checkValues' => Arr::get($defaultValue, 'setting.' . config('consts.document_request_alls.RESERVATION_INFO'), null),
              'parent' => $parent,
              'childs' => $childs,
              'name' => config('consts.document_request_alls.RESERVATION_INFO'),
              'idPrefix' => 'ri',
            ])
          @endforeach
        </ul>
      ...
      <li>
        <h3>{{ __("values.document_request_alls.breakdown_price") }}</h3>
        <ul>
          <input type="hidden" name="setting[{{ config('consts.document_request_alls.BREAKDOWN_PRICE') }}]" />{{-- チェックなしでも値が更新されるようにhiddenで初期化 --}}
          @foreach(config('consts.document_request_alls.BREAKDOWN_PRICE_LIST') as $parent => $childs)
            @include('staff.document_category.common._setting_row', [
              'checkValues' => Arr::get($defaultValue, 'setting.' . config('consts.document_request_alls.BREAKDOWN_PRICE'), null),
              'parent' => $parent,
              'childs' => $childs,
              'name' => config('consts.document_request_alls.BREAKDOWN_PRICE'),
              'idPrefix' => 'o',
            ])
          @endforeach
        </ul>

...
<main>
  ...
  <form method="POST" id="optionForm" action="{{ route("staff.master.subject.option.update",[$agencyAccount, $subjectOption]) }}">
    @csrf
    @method('PUT')
    ...
    <div id="inputArea">
          ...
          <div class="selectBox">
            <select name="category" disabled>
              @foreach($formSelects['subjectCategories'] as $code => $str)
                <option value="{{ $code }}" @if($subjectOption->subject_category->code === $code) selected @endif>{{ $str }}</option>
              @endforeach
            </select>
          </div>
        </li>
      ...
      @include("staff.subject.common.option_form", ['editMode' => 'edit'])
...
<script src="{{ mix('/staff/js/subject_option-edit.js') }}"></script>
@endsection
...
@section('modal')
  @include('staff.common.modal_delete', [
    'title' => 'この項目を削除しますか？',
    'actionUrl' => route('staff.master.subject.option.destroy', [

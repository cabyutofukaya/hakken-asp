<div class="documentPrint">
  ...
  <div class="dcHead">
    <div>
      @if(check_business_form_pdf_item($value, "宛名", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK'))){{-- 宛名エリア --}}
        ...
        @endif
      @endif {{-- 宛名エリア ここまで --}}
      @if(check_business_form_pdf_item($value, "予約情報(件名・期間・参加者)", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK'))){{-- 予約情報(件名・期間・参加者)エリア --}}
        <p class="dispTitle">
          @if(check_business_form_pdf_item($value, "件名", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
            件名 {{ Arr::get($value, 'name') }}
          @endif
        </p>
        <p class="dispPeriod">
          @if(check_business_form_pdf_item($value, "期間", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
        ...
        <p class="dispParticipant">
          @if(check_business_form_pdf_item($value, "代表者", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))

            @if(Arr::get($value, 'representative.name'))
              代表者 {{ $value['representative']['name'] }} 
            @endif

            @if(check_business_form_pdf_item($value, "代表者(ローマ字)", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))

              @if(Arr::get($value, 'representative.name_roman'))
                (
                  @if(check_business_form_pdf_item($value, "代表者(ローマ字)_Mr/Ms", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
            ...
            @if(Arr::get($value, 'representative.name') && check_business_form_pdf_item($value, "代表者_代表者(敬称)", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
              様
            @endif {{-- 代表者敬称 --}}
            <br>
          @endif {{-- 代表者 --}}

          @if(check_business_form_pdf_item($value, "参加者", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')) && Arr::get($formSelects, "participants"))
            参加者 @foreach($formSelects['participants'] as $participant)
              {{ $participant['name'] }} 

              @if(check_business_form_pdf_item($value, "参加者(ローマ字)", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')) && $participant['name_roman'])
                (
                  @if(check_business_form_pdf_item($value, "参加者(ローマ字)_Mr/Ms", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
              ...
              @if(check_business_form_pdf_item($value, "参加者_参加者(敬称)", 'document_setting.setting.'.config('consts.document_quotes.RESERVATION_INFO')))
               様
              @endif {{-- 参加者敬称 --}}
    ...
    <div class="dispCorp">
      ...
      @if(check_business_form_pdf_item($value, "検印欄", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK'))){{-- 検印欄エリア --}}
        <div class="dispStump">
          <ul>
  ...
  @if(check_business_form_pdf_item($value, "案内文", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK')) && Arr::get($value, 'document_setting.information')){{-- 案内文 --}}
    <p class="dispAnounce">{!! nl2br(e(Arr::get($value, 'document_setting.information'))) !!}</p>
  @endif

  @if(check_business_form_pdf_item($value, "代金内訳", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK'))){{-- 代金内訳 --}}
    <div class="dispPrice">
      <h3>代金内訳</h3>
      <table>
        <thead>
  ...
  @if(check_business_form_pdf_item($value, "航空券情報", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK')) && $airticketPrices){{-- 航空券情報 --}}
      ...
      <h3>航空券情報</h3>
        <table>
          <thead>
            <tr>
              <th>氏名</th>
              @if(check_business_form_pdf_item($value, "座席・クラス", 'document_setting.setting.'.config('consts.document_quotes.AIR_TICKET_INFO')))
                <th>座席/クラス</th>
              @endif
              @if(check_business_form_pdf_item($value, "航空会社", 'document_setting.setting.'.config('consts.document_quotes.AIR_TICKET_INFO')))
                <th>航空会社</th>
              @endif
              @if(check_business_form_pdf_item($value, "REF番号", 'document_setting.setting.'.config('consts.document_quotes.AIR_TICKET_INFO')))
                <th>REF番号</th>
              @endif
            </tr>
          </thead>
  ...
  @if(check_business_form_pdf_item($value, "ホテル情報", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK')) && $hotelInfo){{-- ホテル情報 --}}
    <div class="dispHotel">
      <h3>宿泊施設情報</h3>
        <table>
          <thead>
  ...
  @if(check_business_form_pdf_item($value, "ホテル連絡先", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK')) && $hotelContacts){{-- ホテル連絡先 --}}
  ...
  @if(check_business_form_pdf_item($value, "備考", 'document_setting.setting.'.config('consts.document_quotes.DISPLAY_BLOCK'))){{-- 備考 --}}

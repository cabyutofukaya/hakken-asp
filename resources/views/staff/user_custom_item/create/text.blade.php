...
<main>
  <div id="pageHead">
    <h1><span class="material-icons">playlist_add</span>テキスト項目追加</h1>
    <ol class="breadCrumbs">
      <li><a href="{{ route('staff.system.custom.index', ['agencyAccount' => $agencyAccount, 'tab' => $defaultUserCustomCategoryCode]) }}">カスタム項目</a></li>
      <li><span>テキスト項目追加</span></li>
    </ol>
  </div>

  <form method="post" action="{{ route('staff.system.custom.text.store', [$agencyAccount]) }}">
    @csrf
...
<script src="{{ mix('/staff/js/user_custom_item-create_text.js') }}"></script>
@endsection

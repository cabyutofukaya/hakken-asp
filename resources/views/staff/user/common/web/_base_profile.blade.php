<div id="inputArea">
  ...
  <ul class="sideList">
    <li class="wd40"><span class="inputLabel">氏名</span>
      <input type="text" name="userable[name]" value="{{ $defaultValue['userable']['name'] ?? null }}" placeholder="例）山田 太郎" maxlength="18" disabled>
    </li>
    <li class="wd40"><span class="inputLabel">氏名(カナ)</span>
      <input type="text" name="userable[name_kana]" value="{{ $defaultValue['userable']['name_kana'] ?? null }}" placeholder="例）ヤマダ タロウ" maxlength="32" disabled>
    </li>
    <li class="wd40 mr00"><span class="inputLabel">氏名(ローマ字)</span>
      <input type="text" name="userable[name_roman]" value="{{ $defaultValue['userable']['name_roman'] ?? null }}" placeholder="例）YAMADA TAROU" maxlength="32" disabled>
    </li>
  </ul>
  <ul class="sideList">
    <li class="wd20">
      ...
      <ul class="baseRadio sideList half mt10">
        @foreach($formSelects['sexes'] as $val => $str)
          <li>
            <input type="radio" id="sex{{ $val }}" name="userable[sex]" value="{{ $val }}" @if(Arr::get($defaultValue, 'userable.sex', config('consts.users.DEFAULT_SEX')) === $val) checked @endif disabled>
    ...
    <li class="wd60"><span class="inputLabel">生年月日</span>
      <div class="selectSet wd100">
        <div class="selectBox wd40 mr10">
          <select name="userable[birthday_y]" disabled>
        ...
        <div class="selectBox wd30 mr10">
          <select name="userable[birthday_m]" disabled>
        ...
        <div class="selectBox wd30">
          <select name="userable[birthday_d]" disabled>
  ...
  <ul class="sideList half" id="disableContactInputArea" 
  ...
  <ul class="baseList">
    <li class="wd40">
      <span class="inputLabel">郵便番号</span>
      <div class="buttonSet">
        <input type="text" name="userable[zip_code]" class="wd60" value="{{ $defaultValue['userable']['zip_code'] ?? null }}" disabled>
        <button class="orangeBtn wd40" disabled>検索</button>
      </div>
    </li>
    <li>
      ...
      <div class="selectSet">
        <div class="selectBox wd20">
          <select name="userable[prefecture_code]" disabled>
            @foreach($formSelects['prefectures'] as $val => $str)
              <option value="{{ $val }}" @if(Arr::get($defaultValue, 'userable.prefecture_code', "") == $val) selected @endif>{{ $str }}</option>
            @endforeach
          </select>
          </div>
        <input type="text" name="userable[address1]" class="wd80" value="{{ $defaultValue['userable']['address1'] ?? null }}" disabled>
      </div>
    </li>
    <li><span class="inputLabel">ビル・建物名</span>
      <input type="text" name="userable[address2]" value="{{ $defaultValue['userable']['address2'] ?? null }}" disabled>
    </li>
  </ul>
  ...
  <ul class="sideList">
    <li><span class="inputLabel">旅券番号</span>
      <input type="text" name="userable[passport_number]" value="{{ $defaultValue['userable']['passport_number'] ?? null }}" maxlength="32" disabled>
    </li>
    <li>
      <span class="inputLabel">旅券発行日</span>
      <div class="calendar">
        <input type="text" name="userable[passport_issue_date]" value="{{ $defaultValue['userable']['passport_issue_date'] ?? null }}" autocomplete="off" disabled>
      </div>
    </li>
    <li>
      <span class="inputLabel">旅券有効期限</span>
      <div class="calendar">
        <input type="text" name="userable[passport_expiration_date]" value="{{ $defaultValue['userable']['passport_expiration_date'] ?? null }}" autocomplete="off" disabled>
      </div>
    </li>
    <li>
      <span class="inputLabel">旅券発行国</span>
      <div class="selectBox">
        <select name="userable[passport_issue_country_code]" disabled>
    ...
    <li class="mr00">
      ...
      <div class="selectBox">
        <select name="userable[citizenship_code]" disabled>
          @foreach($formSelects['countries'] as $val => $str)

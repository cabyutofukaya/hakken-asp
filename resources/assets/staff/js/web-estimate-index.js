...
const EstimateList = ({ searchParam, formSelects, consts }) => {
    ...
    const [sortParam, setSortParam] = useState({
        ...
        "web_reserve_ext.agency_unread_count": "desc",
        web_online_schedule: "desc",
        estimate_status: "desc"
    });

    const [lists, setLists] = useState([]); //一覧
    const [deleteNumber, setDeleteNumber] = useState(null); // 削除対象ハッシュID
    ...
    const fetch = async () => {
        ...
        const response = await axios
            .get(`/api/${agencyAccount}/web/estimate/list`, {
                params: {
                    ...searchParam,
                    page: page,
                    sort: sort,
    ...
    const handleDelete = async e => {
        ...
        const response = await axios
            .delete(`/api/${agencyAccount}/web/estimate/${deleteNumber}`)
            .finally(() => {
                    ...
                    <thead>
                        <tr>
                            <th
                                className="sort"
                                onClick={e =>
                                    handleSortClick("estimate_reqeust_number")
                                }
                            >
                                <span>見積/依頼番号</span>
                            </th>
                        ...
                        {!isLoading &&
                            ...
                            lists.map((row, index) => (
                                ...
                                >
                                        ...
                                        <span
                                            className={classNames("status", {
                                                red:
                                                    row.web_reserve_ext
                                                        .estimate_status ==
                                                    consts.estimateStatuses
                                                        .estimate_status_consult,
                                                green:
                                                    row.web_reserve_ext
                                                        .estimate_status ==
                                                    consts.estimateStatuses
                                                        .estimate_status_estimate,
                                                orange:
                                                    row.web_reserve_ext
                                                        .estimate_status ==
                                                    consts.estimateStatuses
                                                        .estimate_status_reserve_request
                                            })}
                                        >
                                            {row.web_reserve_ext
                                    ...
                                    />
                                    <td className="txtalc">
                                        {row.web_reserve_ext.web_online_schedule
                                            ?.consult_date && (
                                            <VideoTitArea
                                                webReserveExt={
                                                    row.web_reserve_ext

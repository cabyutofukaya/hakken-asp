...
const DepositBatchModal = ({
    ...
    const handleClick = e => {
        ...
        const batch = async () => {
            ...
            // POST用に処理対象ID(reserve_invoice_id)と更新日時のデータをまとめる
            let data = [];
            const targetRow = doneLists.map((reserveInvoiceId, index) => {
                let targetRow = _.find(invoiceLists, function(row) {
                    return row.reserve_invoice_id == reserveInvoiceId;
                });

                let tmp = {};
                tmp["id"] = reserveInvoiceId;
            ...
            if (mounted.current && response?.data?.data) {
                dataDispatch({
                    type: MANAGEMENT_INVOICE.DEPOSIT_BATCED,
                            ...
                            <div className="calendar">
                                <Flatpickr
                                    ...
                                    onChange={(date, dateStr) => {
                                        dataDispatch({
                                            type:
                                                MANAGEMENT_INVOICE.CHANGE_DEPOSITBATCH_INPUT,
                                            payload: {
                                                name: "deposit_date",
                                                value: dateStr
                                            }

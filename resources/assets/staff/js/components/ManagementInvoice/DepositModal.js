...
import { MANAGEMENT_INVOICE } from "../../actions";
...
/**
 * 請求管理一覧ページ（一括請求と通常請求が混ざったリスト）の入金モーダル
 *
 * @param {*} param0
 * @returns
 */
const DepositModal = ({
    ...
    const [deleteDepositlId, setDeleteDepositlId] = useState(null); // 削除対象の入金ID（一括請求用の入金 or 通常請求用の入金）

    // 未入金額を計算。TODO 依存オブジェクトcurrentInvoiceDataの扱いを再考
    const notDepositedAmount = useMemo(() => {
        return (
            currentInvoiceData.amount_total -
            _.sumBy(currentInvoiceData.combination_deposits, "amount")
        );
    }, [currentInvoiceData]);
    ...
    const handleNotDepositedReflect = e => {
        dataDispatch({
            type: MANAGEMENT_INVOICE.CHANGE_DEPOSIT_INPUT,
            payload: {
                name: "amount",
    ...
    const handleDelete = async () => {
        // 削除処理
        if (depositData.is_pay_altogether === null) return; // 一括請求フラグが未設定
        if (!mounted.current) return;
        if (isDepositDeleting) return;

        setIsDepositDeleting(true);

        // API URLを一括請求と通常請求で分ける
        const apiUrl = depositData.is_pay_altogether
            ? `/api/${agencyAccount}/management/bundle_deposit/${deleteDepositlId}`
            : `/api/${agencyAccount}/management/deposit/${deleteDepositlId}`;

        const response = await axios
            .delete(apiUrl, {
                data: {
                    list_type: listTypes.list_type_index //レスポンス時のリストタイプ
                }
            })
            .finally(() => {
        ...
        if (mounted.current && response?.data?.data) {
            dataDispatch({
                type: MANAGEMENT_INVOICE.DEPOSIT_DELETED,
    ...
    const handleDeposit = e => {
        ...
        const depositRegist = async () => {
            ...
            // API URLを一括請求と通常請求で分ける
            const apiUrl = depositData.is_pay_altogether
                ? `/api/${agencyAccount}/management/deposit/reserve_bundle_invoice/${depositData?.reserve_bundle_invoice_id}`
                : `/api/${agencyAccount}/management/deposit/reserve_invoice/${depositData?.reserve_invoice_id}`;

            // パラメータを一括請求と通常請求で分ける（同時編集チェックのための請求レコード更新日時）
            const param = depositData.is_pay_altogether
                ? {
                ...
                : {
                      list_type: listTypes.list_type_index,
                      reserve_invoice: {
                          updated_at: currentInvoiceData?.updated_at
                      }
                  };
            ...
            if (mounted.current && response?.data?.data) {
                dataDispatch({
                    type: MANAGEMENT_INVOICE.DEPOSIT_REGISTED,
                    payload: response.data.data
                });
            }
        };
    ...
    return (
        ...
        >
            ...
            <div className="modal__content">
                ...
                {currentInvoiceData?.combination_deposits && (
                    <DepositHistory
                        deposits={currentInvoiceData.combination_deposits}
                        ...
                        <div className="buttonSet">
                            <OnlyNumberInput
                                ...
                                handleChange={e => {
                                    dataDispatch({
                                        type:
                                            MANAGEMENT_INVOICE.CHANGE_DEPOSIT_INPUT,
                                        payload: {
                                            name: e.target.name,

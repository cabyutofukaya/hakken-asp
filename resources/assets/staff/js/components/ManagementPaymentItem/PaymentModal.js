...
const PaymentModal = ({
    ...
    const handleDelete = async () => {
        ...
        if (mounted.current && response?.data?.data) {
            dataDispatch({
                type: MANAGEMENT_PAYMENT_ITEM.WITHDRAWAL_DELETED,
    ...
    const handleWithdrawal = e => {
        ...
        const withdrawalRegist = async () => {
            ...
            const response = await axios
                .post(
                    `/api/${agencyAccount}/management/withdrawal/account_payable_item/${currentPaymentData?.id}`,
                    {
                        ...withdrawalData,
                        account_payable_item: {
                            updated_at: currentPaymentData?.updated_at
            ...
            if (mounted.current && response?.data?.data) {
                dataDispatch({
                    type: MANAGEMENT_PAYMENT_ITEM.WITHDRAWAL_REGISTED,
                    payload: response.data.data
                });
            }
        };
                        ...
                        <div className="buttonSet">
                            <OnlyNumberInput
                                ...
                                handleChange={e => {
                                    dataDispatch({
                                        type:
                                            MANAGEMENT_PAYMENT_ITEM.CHANGE_WITHDRAWAL_INPUT,
                                        payload: {
                                            name: e.target.name,
